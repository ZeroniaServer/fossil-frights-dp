...
execute as @e[type=minecraft:item_display,tag=ff_door,tag=ff_door.automatic] at @s align xyz positioned ~0.5 ~ ~0.5 if entity @a[distance=..3,predicate=fossil_frights:location/near_door] run scoreboard players operation @s ff_automatic_door_close_timestamp = #gametime_bumped ff_automatic_door_close_timestamp
