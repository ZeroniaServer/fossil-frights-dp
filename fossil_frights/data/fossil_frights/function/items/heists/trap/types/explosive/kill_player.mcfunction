...
execute if data storage fossil_frights:trap_state placer_name run tellraw @a {translate:"ff.death.explosive_trap.player",with:[{selector:"@s"},{nbt:"placer_name",storage:"fossil_frights:trap_state",interpret:true}]}
