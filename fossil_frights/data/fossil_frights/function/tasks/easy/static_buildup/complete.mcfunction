...
execute if score $static_buildup_sel ff_task_state matches 1 if score $static_buildup_done ff_task_state matches 0 as @a[tag=ff_frights_feedback_viewer] at @s run particle minecraft:end_rod ~ ~1 ~ 0.35 0.6 0.35 0.05 45 force
execute if score $static_buildup_sel ff_task_state matches 1 if score $static_buildup_done ff_task_state matches 0 as @a[tag=ff_frights_feedback_viewer] at @s run playsound minecraft:entity.lightning_bolt.thunder master @s ~ ~ ~ 0.7 1.8
