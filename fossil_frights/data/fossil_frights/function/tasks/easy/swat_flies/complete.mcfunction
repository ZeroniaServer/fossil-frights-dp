...
execute if score $swat_flies_sel ff_task_state matches 1 if score $swat_flies_done ff_task_state matches 0 run data modify storage fossil_frights:tasks complete.task_name set value "swat_flies"
execute if score $swat_flies_sel ff_task_state matches 1 if score $swat_flies_done ff_task_state matches 0 at @s run playsound minecraft:entity.player.attack.sweep master @a[tag=ff_frights_feedback_viewer,distance=..12] ~ ~ ~ 0.8 1.3
execute if score $swat_flies_sel ff_task_state matches 1 if score $swat_flies_done ff_task_state matches 0 positioned -13 95 54 run playsound minecraft:entity.bee.hurt master @a[tag=ff_frights_feedback_viewer,distance=..12] ~ ~ ~ 0.5 1.6
