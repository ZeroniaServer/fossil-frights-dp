...
execute if score $toilet_clog_sel ff_task_state matches 1 if score $toilet_clog_done ff_task_state matches 0 as @a[tag=ff_frights_feedback_viewer] at @s run playsound fossil-frights:task.toilet_clog master @s ~ ~ ~ 0.9 1.0
execute if score $toilet_clog_sel ff_task_state matches 1 if score $toilet_clog_done ff_task_state matches 0 as @a[tag=ff_frights_feedback_viewer] at @s run playsound minecraft:entity.generic.splash master @s ~ ~ ~ 0.9 1.2
...
execute if score $toilet_clog_sel ff_task_state matches 1 if score $toilet_clog_done ff_task_state matches 0 run clear @a[team=ff_guard] minecraft:written_book[minecraft:custom_data~{ff_task_book:"toilet_clog"}] 1
