...
execute if score $count_shells_sel ff_task_state matches 1 if score $count_shells_done ff_task_state matches 0 run data modify storage fossil_frights:tasks complete.task_name set value "count_shells"
execute if score $count_shells_sel ff_task_state matches 1 if score $count_shells_done ff_task_state matches 0 as @a[tag=ff_frights_feedback_viewer] at @s run playsound minecraft:block.conduit.ambient.short master @s ~ ~ ~ 0.8 1.4
