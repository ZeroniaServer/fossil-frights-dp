...
execute if score $tnt_test_sel ff_task_state matches 1 if score $tnt_test_done ff_task_state matches 0 positioned 44 64 82 run playsound minecraft:entity.tnt.primed master @a[tag=ff_frights_feedback_viewer,distance=..24] ~ ~ ~ 1.0 1.0
execute if score $tnt_test_sel ff_task_state matches 1 if score $tnt_test_done ff_task_state matches 0 run summon minecraft:tnt 44.5 64.25 82.5 {Fuse:15,fuse:15,ExplosionPower:0,explosion_power:0,NoGravity:true,Tags:["ff_tnt_test"]}
