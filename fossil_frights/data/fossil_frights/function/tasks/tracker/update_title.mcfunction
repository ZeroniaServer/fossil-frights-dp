...
execute if score $run_multiplayer ff_game_state matches 0 as @a[team=ff_guard,limit=1] run function fossil_frights:player/util/write_username {to:"storage fossil_frights:tasks tracker_title.name"}
...
execute unless data storage fossil_frights:tasks tracker_title.name unless data storage fossil_frights:tasks tracker_title.name2 run scoreboard objectives modify ff_task_tracker displayname [{"translate":"ff.sidebar.tasks","color":"yellow","italic":false}]
tag @a remove ff_title_first
