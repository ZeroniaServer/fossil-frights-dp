...
execute if predicate fossil_frights:game_state/heist_mode_active if score $heist_round_active ff_game_state matches 1 unless entity @a[team=ff_guard] unless entity @a[team=ff_thief] unless entity @e[type=minecraft:marker,tag=ff_rejoin_marker,tag=!ff_rejoin_ghosted] run function fossil_frights:game/reset
