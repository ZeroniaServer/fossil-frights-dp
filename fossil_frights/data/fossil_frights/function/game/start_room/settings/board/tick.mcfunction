...
execute unless predicate fossil_frights:world/packs_enabled/plugin as @a[team=ff_guard] if predicate fossil_frights:player/start_room/settings/looking_at_chat run title @s actionbar {translate:"ff.settings.hover.not_available",color:"red",italic:false}
