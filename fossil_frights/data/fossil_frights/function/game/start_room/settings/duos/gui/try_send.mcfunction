...
execute store result score $invite_uuid_0 ff_active_uuid_0 run data get storage fossil_frights:invite selected.u0 1
execute store result score $invite_uuid_1 ff_active_uuid_1 run data get storage fossil_frights:invite selected.u1 1
execute store result score $invite_uuid_2 ff_active_uuid_2 run data get storage fossil_frights:invite selected.u2 1
execute store result score $invite_uuid_3 ff_active_uuid_3 run data get storage fossil_frights:invite selected.u3 1
execute as @a[tag=!ff_invite_caller] if score @s ff_active_uuid_0 = $invite_uuid_0 ff_active_uuid_0 if score @s ff_active_uuid_1 = $invite_uuid_1 ff_active_uuid_1 if score @s ff_active_uuid_2 = $invite_uuid_2 ff_active_uuid_2 if score @s ff_active_uuid_3 = $invite_uuid_3 ff_active_uuid_3 run function fossil_frights:game/start_room/settings/duos/gui/send_to_target
