...
execute as @a[tag=!ff_invite_self] run function fossil_frights:game/start_room/settings/duos/gui/add_entry
tag @s remove ff_invite_self
execute unless data storage fossil_frights:invite entries[0] run function fossil_frights:messages/duos/no_players_online
execute unless data storage fossil_frights:invite entries[0] run return 0
scoreboard players enable @s ff_invite_sel
function fossil_frights:game/start_room/settings/duos/gui/show with storage fossil_frights:invite
