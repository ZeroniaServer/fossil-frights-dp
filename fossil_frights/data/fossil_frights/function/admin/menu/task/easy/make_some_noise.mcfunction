...
execute if score $make_some_noise_sel ff_task_state matches 1 run scoreboard players set $admin_task_was_selected ff_gui 1
execute if score $admin_task_was_selected ff_gui matches 1 run function fossil_frights:admin/menu/task/complete_macro {difficulty:"easy",task:"make_some_noise"}
execute unless score $admin_task_was_selected ff_gui matches 1 run function fossil_frights:tasks/easy/make_some_noise/selected
