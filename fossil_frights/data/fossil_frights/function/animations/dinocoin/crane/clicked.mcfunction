...
execute if score #crane_coin_count ff_game_state matches 2.. positioned 52 68 60 run tellraw @s {"translate":"ff.sculkadillo.thanks","with":[{"translate":"ff.sculkadillo","color":"dark_aqua"},{"translate":"%s %s", "with": [{"translate":"ff.dinocoins.number","color":"gold","with":["2"]},{"atlas":"blocks","sprite":"fossil-frights:items/misc/dinocoin"}]},{"translate":"ff.seconds.number","color":"red","with":["10"]}]}
