...
execute if score $sarcophagus_timer ff_game_state matches 0 positioned 80 75 81 if entity @e[type=minecraft:item,distance=..1.5,predicate=fossil_frights:entity/contents/vanilla/gold_ingot,predicate=fossil_frights:entity/contents/count/1,limit=1] run function fossil_frights:animations/cubekoin/sarcophagus/payment/pay
