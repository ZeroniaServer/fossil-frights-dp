...
execute if score $crane_wait ff_game_state matches 1.. at @e[limit=1,type=minecraft:item_display,tag=ff_deep_dark_elevator] positioned ~-0.5 ~1 ~-0.5 if entity @a[limit=1,dx=0,predicate=fossil_frights:entity/on_ground] run function fossil_frights:animations/dinocoin/crane/start_ride
