...
execute if score #crane_coin_count ff_game_state matches ..1 if score @s ff_msg_cooldown matches 0 run title @s actionbar [{"translate":"ff.dinocoins.required","color":"red","with":[{"translate":"%s %s","color":"white","with":[{"translate":"ff.dinocoin.number","with":["2"],"color":"yellow"},{"atlas":"blocks","sprite":"fossil-frights:custom/general_items/dinocoin"}]}]}]
