...
execute positioned 39 81 62 if entity @e[type=minecraft:marker,tag=anvil,tag=!ff_anvil_busy,distance=..0.1,limit=1] if items entity @s weapon.mainhand minecraft:iron_ingot if data entity @s {SelectedItem:{components:{"minecraft:lore":[{text:"",extra:["scrap_metal"]}]}}} run function fossil_frights:animations/anvil/clicked/iron_ingot
