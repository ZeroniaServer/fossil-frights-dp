...
execute if score @s ff_glowtrap_age matches 50.. if entity @e[type=minecraft:item,limit=1,distance=..0.75] run function fossil_frights:items/heists/trap/types/jelly/trigger_item
execute positioned ~-1.5 ~-1.5 ~-1.5 unless entity @a[limit=1,team=ff_thief,gamemode=!spectator,dx=3,dy=3,dz=3] run return 0
function fossil_frights:items/heists/trap/types/jelly/trigger
