...
$loot replace entity @e[limit=1,dx=0,type=minecraft:item,tag=ff_summon_item_new] contents loot $(loot_table)
execute as @e[limit=1,dx=0,type=minecraft:item,tag=ff_summon_item_new] if items entity @s contents *[custom_data={ff_summon_item_delete:true}] run kill @s
tag @e[limit=1,dx=0,type=minecraft:item,tag=ff_summon_item_new] remove ff_summon_item_new
