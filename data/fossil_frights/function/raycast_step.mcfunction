...
execute if score @s ff_lock_look matches 1 as @e[type=minecraft:item_frame,distance=..0.35,sort=nearest,limit=1,nbt={Item:{id:"minecraft:iron_ingot"}}] at @s run scoreboard players set @e[type=minecraft:interaction,tag=ff_lock_click,distance=..0.1,limit=1] ff_lock_click_ttl 5
