...
execute as @e[type=minecraft:pufferfish,tag=ff_pufferfish_fright,limit=1] at @s if entity @a[tag=ff_active,distance=..6] as @a[tag=ff_active,distance=..6,sort=nearest,limit=1] at @s unless block ~ ~ ~ minecraft:water run scoreboard players set $puffer_timer ff_game_state 0
