...
execute as @a[team=ff_thief,gamemode=!spectator] run title @s times 0 20 8
execute as @a[team=ff_thief,gamemode=!spectator] run title @s subtitle [{"text":"+$","color":"yellow","italic":false},{"score":{"name":"@s","objective":"ff_heist_reward"},"color":"yellow","italic":false}]
execute as @a[team=ff_thief,gamemode=!spectator] run title @s title {"text":" "}
