...
execute if score $game_running ff_game_state matches 1 if score $day_active ff_day matches 0 unless score $party_mode_active ff_game_state matches 1 unless score $heist_mode_active ff_game_state matches 1 if score $heist_button_cur ff_gui matches 1 unless score $heist_button_prev ff_gui matches 1 positioned 13 71 20 as @a[tag=ff_active,distance=..8,limit=1,sort=nearest] run function fossil_frights:messages/settings/heists_dialog
