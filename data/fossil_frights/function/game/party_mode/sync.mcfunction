...
execute if score $run_multiplayer ff_game_state matches 1 if score $party_mode_active ff_game_state matches 1 unless entity @a[limit=1,tag=ff_active] run return run function fossil_frights:game/reset
execute if score $run_multiplayer ff_game_state matches 0 unless entity @a[limit=1,tag=ff_active] run function fossil_frights:game/reset/active_disconnect
