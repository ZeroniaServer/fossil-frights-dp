...
execute if score $skincare_routine_done ff_task_state matches 1 run return 0
execute unless entity @e[limit=1,tag=ff_skincare_routine_potion,x=-1,y=102,z=14,dx=2,dy=5,dz=6] run function fossil_frights:tasks/hard/skincare_routine/pending_clear
execute if entity @e[limit=1,tag=ff_skincare_routine_potion,x=-1,y=102,z=14,dx=2,dy=5,dz=6] run schedule function fossil_frights:tasks/hard/skincare_routine/check_splash 5t
