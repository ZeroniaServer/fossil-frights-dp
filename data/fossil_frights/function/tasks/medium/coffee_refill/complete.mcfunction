...
execute if score $coffee_refill_sel ff_task_state matches 1 if score $coffee_refill_done ff_task_state matches 0 run data modify storage fossil_frights:tasks complete.task_name set value "coffee_refill"
...
execute if score $coffee_refill_sel ff_task_state matches 1 if score $coffee_refill_done ff_task_state matches 0 positioned -21.5 70.5 35.5 run playsound minecraft:block.brewing_stand.brew block @a[team=ff_guard,distance=..18] ~ ~ ~ 0.8 1.15
execute if score $coffee_refill_sel ff_task_state matches 1 if score $coffee_refill_done ff_task_state matches 0 positioned -21.5 70.5 35.5 run playsound minecraft:block.fire.extinguish block @a[team=ff_guard,distance=..18] ~ ~ ~ 0.28 1.8
...
execute if score $coffee_refill_sel ff_task_state matches 1 if score $coffee_refill_done ff_task_state matches 0 run clear @a[team=ff_guard] minecraft:written_book[minecraft:custom_data~{ff_task_book:"coffee_refill"}] 1
...
function fossil_frights:tasks/medium/coffee_refill/cleanup
