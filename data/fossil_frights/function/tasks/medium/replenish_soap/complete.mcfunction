...
execute if score $replenish_soap_sel ff_task_state matches 1 if score $replenish_soap_done ff_task_state matches 0 run summon minecraft:item_display -22.5 95.1 19.5 {Tags:["ff_replenish_soap_display"],item:{id:"minecraft:paper",count:1},item_display:"fixed",brightness:{sky:15,block:15},transformation:{right_rotation:[0f,0f,0f,1f],left_rotation:[0.7071068f,0f,0f,0.7071068f],translation:[0f,0f,0f],scale:[0.55f,0.55f,0.55f]}}
execute if score $replenish_soap_sel ff_task_state matches 1 if score $replenish_soap_done ff_task_state matches 0 run data modify entity @e[type=minecraft:item_display,tag=ff_replenish_soap_display,sort=nearest,limit=1,x=-22.5,y=95.1,z=19.5,distance=..0.2] item set from entity @s Item
