...
execute unless items entity @s weapon.mainhand minecraft:rotten_flesh unless data entity @s {Inventory:[{Slot:-106b,id:"minecraft:rotten_flesh"}]} run return 0
...
execute if score $feed_the_fish_sel ff_task_state matches 1 if score $feed_the_fish_done ff_task_state matches 0 run clear @a[tag=ff_active] minecraft:written_book[minecraft:custom_data~{ff_task_book:"feed_the_fish"}] 1
