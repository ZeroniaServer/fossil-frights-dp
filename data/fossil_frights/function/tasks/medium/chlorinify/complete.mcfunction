...
execute if score $chlorinify_sel ff_task_state matches 1 if score $chlorinify_done ff_task_state matches 0 at @s run playsound minecraft:entity.generic.splash master @a[tag=ff_active,distance=..16] ~ ~ ~ 0.9 1.2
...
execute if score $chlorinify_sel ff_task_state matches 1 if score $chlorinify_done ff_task_state matches 0 run clear @a[tag=ff_active] minecraft:written_book[minecraft:custom_data~{ff_task_book:"chlorinify"}] 1
