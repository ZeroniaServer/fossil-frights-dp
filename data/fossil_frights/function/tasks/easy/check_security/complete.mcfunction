execute if score $check_security_sel ff_task_state matches 1 if score $check_security_done ff_task_state matches 0 run data modify storage fossil_frights:tasks complete.task_name set value "Check Security"
...
execute if score $check_security_sel ff_task_state matches 1 if score $check_security_done ff_task_state matches 0 if entity @s[type=minecraft:player] run clear @s minecraft:written_book[minecraft:custom_data~{ff_task_book:"check_security"}] 1
...
execute as @a[tag=ff_active,sort=nearest,limit=1] at @s run function fossil_frights:tasks/easy/check_security/forced_spectate_start
