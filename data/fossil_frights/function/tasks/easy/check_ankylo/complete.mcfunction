...
execute if score $check_ankylo_sel ff_task_state matches 1 if score $check_ankylo_done ff_task_state matches 0 run data modify storage fossil_frights:tasks complete.task_name set value "Ccheck_anklyo"
execute if score $check_ankylo_sel ff_task_state matches 1 if score $check_ankylo_done ff_task_state matches 0 as @a[tag=ff_active] at @s run playsound minecraft:entity.camel.ambient master @s ~ ~ ~ 0.9 0.8
