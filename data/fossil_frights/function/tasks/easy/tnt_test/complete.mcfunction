...
execute if score $tnt_test_sel ff_task_state matches 1 if score $tnt_test_done ff_task_state matches 0 positioned 52 63 62 run playsound minecraft:entity.tnt.primed master @a[tag=ff_active,distance=..24] ~ ~ ~ 1.0 1.0
execute if score $tnt_test_sel ff_task_state matches 1 if score $tnt_test_done ff_task_state matches 0 run summon minecraft:tnt 52.5 63.25 62.5 {Fuse:15,fuse:15,ExplosionPower:0.0f,explosion_power:0.0f,NoGravity:1b,Tags:["ff_tnt_test"]}
