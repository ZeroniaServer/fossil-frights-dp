...
execute if score $lock_register_sel ff_task_state matches 1 if score $lock_register_done ff_task_state matches 0 run data modify storage fossil_frights:tasks complete.task_name set value "Lock Register"
execute if score $lock_register_sel ff_task_state matches 1 if score $lock_register_done ff_task_state matches 0 as @a[tag=ff_active] at @s run playsound minecraft:block.chest.locked master @s ~ ~ ~ 0.9 1.0
...
execute if score $lock_register_sel ff_task_state matches 1 if score $lock_register_done ff_task_state matches 0 run clear @a[tag=ff_active] minecraft:written_book[minecraft:custom_data~{ff_task_book:"lock_register"}] 1
